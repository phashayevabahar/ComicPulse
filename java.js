  ...
  try {
    ...
    products.forEach((product) => {
      ...
      card.innerHTML = `
        ...
        product.name
      }" style="max-width:100%; border-radius:8px; margin-bottom:10px;" onerror="this.style.display='none'">
        <h2>${product.name}</h2>
...
function renderCartItems() {
  ...
  if (cartItems.length === 0) {
    cartContainer.innerHTML =
      '<p>Cart is empty.    <a href="index.html">Start to shopping</a></p>';
    updateCartTotal();
    return;
  }
  ...
  cartItems.forEach((item) => {
    ...
    div.innerHTML = `
      ...
      item.name
    }" style="width:120px; height:120px; object-fit: fill; border-radius:6px; margin-right:15px;">
      <div style="flex-grow:1;">
        ...
        <p style="margin:5px 0 0 0; font-size:14px; color:#555;">
          ...
          <button class="increase-qty" data-id="${
            item.id
          }" style="padding: 2px 6px; margin-left: 8px;">+</button>
